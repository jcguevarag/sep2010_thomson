...
datatest = sunpy.map.Map('Stokes_IQUV_averages/hmi.S_90s.20170910_161800_TAI.3I_recentered.medians_4pics.meanvalue.fits').data
...
for ind,item in enumerate(enc_coor):
    ...
    del x2,y2,h
psh_points = np.genfromtxt('hmi_055_psh.csv', delimiter=',') #Points from figure 5 top
psh_points_pol = np.genfromtxt('polarization_v_055_blob.csv', delimiter=',') #Points from figure 5 bottom
...
for inter in Q_region_scc:
    ...
    for i in range(len(inter)):
        for j in range(len(inter[i])):
            if inter[i,j] > I_bkg:
                grid_di_tmp[i,j] = inter[i,j]-Q_bkg
